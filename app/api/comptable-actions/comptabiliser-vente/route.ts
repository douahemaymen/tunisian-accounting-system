...
export async function POST(request: NextRequest) {
  try {
    ...
    const { factureVenteId } = await request.json();

    if (!factureVenteId) {
      return NextResponse.json({ error: 'ID de facture de vente requis' }, { status: 400 });
    }

    // Récupérer la facture de vente avec les informations du client
    const factureVente = await prisma.journalVente.findUnique({
      where: { id: factureVenteId },
      include: {
        client: {
          include: {
            comptable: true
    ...
    if (!factureVente) {
      return NextResponse.json({ error: 'Facture de vente non trouvée' }, { status: 404 });
    }
    ...
    if (isComptable && factureVente.client?.comptable?.userId !== session.user.id) {
      return NextResponse.json({ error: 'Accès non autorisé à cette facture' }, { status: 403 });
    }

    // Vérifier si déjà comptabilisée
    if (factureVente.status === 'COMPTABILISE') {
      return NextResponse.json({
        error: 'Cette facture de vente est déjà comptabilisée'
      }, { status: 400 });
    }

    // Récupérer le plan comptable du comptable
    let planComptable;
    if (factureVente.client?.comptableId) {
      planComptable = await prisma.planComptable.findMany({
        where: { comptableId: factureVente.client.comptableId },
    ...
    const factureData = {
      id: factureVente.id,
      type_facture: factureVente.type_facture,
      clientdefacture: factureVente.clientdefacture,
      date: factureVente.date,
      reference: factureVente.reference,
      total_ht: factureVente.total_ht,
      tva_19: factureVente.tva_19,
      tva_13: factureVente.tva_13,
      tva_7: factureVente.tva_7,
      total_tva: factureVente.total_tva,
      total_ttc: factureVente.total_ttc,
      remise: factureVente.remise,
      timbre_fiscal: factureVente.timbre_fiscal,
      type_journal: 'J_VTE' // Journal de vente
    };

    console.log('💰 Comptabilisation facture vente avec Gemini AI:', {
      factureVenteId,
      client: factureVente.clientdefacture,
      typeFacture: factureVente.type_facture,
      montantTTC: factureVente.total_ttc,
    ...
    console.log('🔍 Vérification équilibrage facture vente:', {
      totalDebit,
      totalCredit,
      difference,
      tolerance,
    ...
    // Transaction pour comptabiliser la facture de vente
    const result = await prisma.$transaction(async (tx) => {
      ...
      const factureUpdated = await tx.journalVente.update({
        where: { id: factureVenteId },
        data: {
          status: 'COMPTABILISE',
          accounting_entries: {
    ...
    console.log('✅ Facture de vente comptabilisée avec succès:', {
      factureVenteId,
    ...
    return NextResponse.json({
      success: true,
      message: 'Facture de vente comptabilisée avec succès',
      ecrituresCount: result.ecrituresCreees.length,
    ...
    console.error('❌ Erreur comptabilisation facture vente:', error);
    ...
    return NextResponse.json({
      error: 'Erreur lors de la comptabilisation de la facture de vente',
