...
export async function POST(request: NextRequest) {
  try {
    ...
    if (facture.client?.comptableId) {
      ...
      // Si pas de comptable assigné, utiliser un plan comptable par défaut ou retourner une erreur
    ...
    console.log('🧠 Génération écritures avec Gemini AI pour:', {
      factureId,
      fournisseur: facture.fournisseur,
      typeJournal: facture.type_facture,
    ...
    if (comptesInvalides.length > 0) {
      console.warn('Comptes invalides détectés:', comptesInvalides);
      return NextResponse.json({
    ...
    return NextResponse.json({
      success: true,
      ecritures: ecrituresCreees.map(e => ({
        id: e.id,
      ...
      message: `${ecrituresCreees.length} écriture(s) comptable(s) générée(s) avec Gemini AI`
    });
    ...
    return NextResponse.json({
      error: 'Erreur lors de la génération des écritures comptables',
